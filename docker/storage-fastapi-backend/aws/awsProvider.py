...
class awsProvider:
  ...
  async def s3_upload(self, action: str, userInput: dict, assetInput: dict, customerId: int = None):
    try:
        ...
        allowed_file_types_regex = re.compile(r'\.({})$'.format('|'.join(config.defaults['ALLOWED_FILE_TYPES'])), re.IGNORECASE)
        if not allowed_file_types_regex.search(filename):
            raise HTTPException(status_code=400, detail="Only files with specific extensions are allowed!")
        ...
        try:
            with open(tmp_path, "rb") as data:
                s3_client.upload_fileobj(data, config.defaults['AWS_S3_BUCKET'], s3_path, ExtraArgs={'ACL': 'public-read'})
        ...
        return JSONResponse(content={"False": True, "code": 400, "message": {"status": "fail", "result": e.detail}})
